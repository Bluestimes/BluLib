...
namespace BluLib
{
	...
	public static class TransformShakeExtension
	{
		...
		{
			if (_activeShakingTransforms == null) _activeShakingTransforms = new();

			BreakShakeIfAny(transform);
			
			Coroutine coroutine = TransformShakeCoroutine(transform, time, shakeBounds, useUnscaledTime, fadeBounds).StartCoroutine();
			_activeShakingTransforms.Add(transform, new(coroutine, transform.position));
		}
		...
		private static void BreakShakeIfAny(Transform transform)
		{
			...
			Tuple<Coroutine, Vector3> shakeData = _activeShakingTransforms[transform];
			BluCoroutines.StopCoroutine(shakeData.Item1);
