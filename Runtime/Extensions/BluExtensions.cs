...
namespace BluLib
{
	public static class BluExtensions
	{
		...
		{
			Vector3 screenPoint = camera.WorldToScreenPoint(source, eye);
		...
		{
			Transform[] allTransforms = source.GetComponentsInChildren<Transform>(true);
			foreach (Transform tf in allTransforms) tf.gameObject.layer = layer;
			return source;
		}
		...
		{
			Transform[] monoBehaviours = Object.FindObjectsOfType<Transform>();
