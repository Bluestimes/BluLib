...
namespace BluLib
{
	public static class BluCollections
	{
		...
		{
			if (!source.ContainsKey(key)) source[key] = new();
			return source[key];
		}
		/// <summary>
		/// Adds a key/value pair to the IDictionary&lt;TKey,TValue&gt; if the
		...
		{
			IEnumerable<double> weights = source.Select(weightSelector).Select(w => w < 0 ? 0 : w);
			IEnumerable<double> weightStages = weights.Select((w, i) => weights.Take(i + 1).Sum());
			double roll = BluCommonConstants.SystemRandom.NextDouble() * weights.Sum();
		...
		public static T[] ExclusiveSample<T>(this IList<T> source, int sampleNumber)
		{
			...
			T[] results = new T[sampleNumber];
			int resultIndex = 0;
			for (int i = 0; i < source.Count && resultIndex < sampleNumber; ++i)
			{
				double probability = (double)(sampleNumber - resultIndex)
                                     / (source.Count - i);
				if (BluCommonConstants.SystemRandom.NextDouble() < probability)
