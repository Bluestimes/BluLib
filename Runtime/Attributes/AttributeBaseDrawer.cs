...
namespace BluLib.Internal
{
	...
	{
		private AttributeBase[] cachedAttributes;

		public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
		{
			CacheAttributes();

			for (int i = cachedAttributes.Length - 1; i >= 0; i--)
			{
				float? overriden = cachedAttributes[i].OverrideHeight();
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			for (int i = cachedAttributes.Length - 1; i >= 0; i--)
			{
				AttributeBase ab = cachedAttributes[i];
		...
		{
			if (cachedAttributes.IsNullOrEmpty())
			{
				cachedAttributes = fieldInfo
