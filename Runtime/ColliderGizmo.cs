...
namespace BluLib
{
	public class ColliderGizmo : MonoBehaviour
	{
		...
		public void Refresh()
		{
			...
			_wireGizmoColor = new(WireColor.r, WireColor.g, WireColor.b, WireColor.a * Alpha);
			_fillGizmoColor = new(FillColor.r, FillColor.g, FillColor.b, FillColor.a * Alpha);
			_centerGizmoColor = new(CenterColor.r, CenterColor.g, CenterColor.b, CenterColor.a * Alpha);
			...
			for (int i = 0; i < colliders2d.Length; i++)
			{
				...
				BoxCollider2D box2d = c as BoxCollider2D;
				if (box2d != null)
				{
					if (_boxColliders2D == null) _boxColliders2D = new();
					_boxColliders2D.Add(box2d);
					_withColliders.Add(box2d.transform);
					continue;
				}

				EdgeCollider2D edge = c as EdgeCollider2D;
				if (edge != null)
				{
					if (_edgeColliders2D == null) _edgeColliders2D = new();
					_edgeColliders2D.Add(edge);
					_withColliders.Add(edge.transform);
					continue;
				}

				CircleCollider2D circle2d = c as CircleCollider2D;
				if (circle2d != null)
				{
					if (_circleColliders2D == null) _circleColliders2D = new();
			...
			for (int i = 0; i < colliders.Length; i++)
			{
				...
				if (box != null)
				{
					if (_boxColliders == null) _boxColliders = new();
				...
				if (sphere != null)
				{
					if (_sphereColliders == null) _sphereColliders = new();
				...
				if (mesh != null)
				{
					if (_meshColliders == null) _meshColliders = new();
		...
		private void DrawEdgeCollider2D(EdgeCollider2D coll)
		{
			...
			for (int i = 0; i < coll.points.Length; i++)
			{
				...
				Vector3 pos = new(collPoint.x * lossyScale.x, collPoint.y * lossyScale.y, 0);
		...
		{
			Transform target = obstacle.transform;
		...
		private void DrawColliders()
		{
			if (DrawCenter)
			{
				...
				foreach (Transform withCollider in _withColliders)
				{
			...
			{
				foreach (EdgeCollider2D edge in _edgeColliders2D)
				{
			...
			{
				foreach (CircleCollider2D circle in _circleColliders2D)
				{
	...
	public class ColliderGizmoEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				{
					ColliderGizmo gizmo = (ColliderGizmo)singleTarget;
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				SerializedProperty customWireColor = serializedObject.FindProperty("CustomWireColor");
				SerializedProperty customFillColor = serializedObject.FindProperty("CustomFillColor");
				SerializedProperty customCenterColor = serializedObject.FindProperty("CustomCenterColor");
