...
namespace BluLib.Internal
{
	public class AssetPresetPreprocessor : AssetPostprocessor
	{
		...
		private void OnPreprocessAsset()
		{
			...
			foreach (ConditionalPreset preset in _preprocessBase.Presets)
			{
		...
		private bool PreloadBase()
		{
			...
			{
				_preprocessBase = BluScriptableObject.LoadAssetsFromResources<AssetsPresetPreprocessBase>().FirstOrDefault();
				if (_preprocessBase == null) _preprocessBase = BluScriptableObject.LoadAssets<AssetsPresetPreprocessBase>().SingleOrDefault();
