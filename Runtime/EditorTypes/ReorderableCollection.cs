...
namespace BluLib.EditorTools
{
	public class ReorderableCollection
	{
		...
		public void Draw()
		{
			...
			{
				Rect headerRect = GUILayoutUtility.GetRect(0, 0, GUILayout.ExpandWidth(true));
		...
		{
			_list = new(property.serializedObject, property, true, displayHeader, withAddButton,
				withRemoveButton);
		...
		private void DrawElement(Rect rect, int index, bool active, bool focused)
		{
			...
			{
				SerializedProperty element = _property.GetArrayElementAtIndex(index);

				if (element.propertyType == SerializedPropertyType.Generic)
				{
					rect.x += 12;
					rect.width -= 14;
					Rect genericsLabel = rect;
