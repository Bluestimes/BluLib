...
namespace BluLib
{
	...
	public class UISizeBy : MonoBehaviour
	{
		...
		private void LateUpdate()
		{
			...
			Vector2 copyFromSize = CopySizeFrom.sizeDelta;
			if (_latestSize == copyFromSize) return;
			_latestSize = copyFromSize;

			Vector2 toSize = _transform.sizeDelta;
			float x = CopyWidth.IsSet ? _latestSize.x + CopyWidth.Value : toSize.x;
			float y = CopyHeight.IsSet ? _latestSize.y + CopyHeight.Value : toSize.y;

			x = MinMaxWidth.GetFixed(x);
			y = MinMaxHeight.GetFixed(y);

			_transform.sizeDelta = new(x, y);
		}
		...
		private void UpdateView()
		{
			...
			Undo.RecordObject(_transform, "UISizeBy.UpdateView");
			LateUpdate();
		}
#endif
	}
