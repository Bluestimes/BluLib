...
#pragma warning restore 0649
		...
		public Vector2 OffscreenOffset
		{
			get
			{
				...
				float minX = anchoredPosition.x + halfWidth;
				float maxX = anchoredPosition.x - halfWidth - Screen.width;
				if (minX < 0) offX = minX;
				else if (maxX > 0) offX = maxX;

				float halfHeight = rect.height / 2;
				float offY = 0f;
				float minY = anchoredPosition.y + halfHeight;
				float maxY = anchoredPosition.y - halfHeight - Screen.height;
				if (minY < 0) offY = minY;
		...
		private void LateUpdate()
		{
			...
			Vector3 followPosition = ToFollow.position.Offset(Offset);
