...
namespace BluLib.Internal
{
	public class SerializedPropertyViewer : EditorWindow
	{
		...
		{
			SerializedPropertyViewer propertyViewer = (SerializedPropertyViewer) GetWindow(typeof(SerializedPropertyViewer));
			propertyViewer.titleContent = new("Property Explorer");
		...
		private readonly List<PropertyData> _propertiesData = new();
		...
		{
			_target = target;
			_targetSO = new(_target);

			if (_richTextStyle == null)
			{
				_richTextStyle = new(EditorStyles.label);
		...
		private void OnGUI()
		{
			...
			bool debug = EditorGUILayout.Toggle("Debug Mode", _debugMode);
			if (debug != _debugMode)
			{
				_debugMode = debug;
				CollectProperties();
			...
			if (GUILayout.Button("Copy To Clipboard"))
			{
				...
				Dictionary<int, string> paddingHash = new();
				string padding = "";
				for (int i = 0; i < 40; i++)
				{
		...
		private void CollectProperties()
		{
			...
			InspectorMode inspectorMode = _debugMode ? InspectorMode.Debug : InspectorMode.Normal;
			PropertyInfo inspectorModeInfo = typeof(SerializedObject).GetProperty("inspectorMode", BindingFlags.NonPublic | BindingFlags.Instance);
			if (inspectorModeInfo != null) inspectorModeInfo.SetValue(_targetSO, inspectorMode, null);


			SerializedProperty iterator = _targetSO.GetIterator();
		...
		{
			string value = AsStringValue(property);
			...
			_propertiesData.Add(new(property.depth, description, propertyId));
		}
