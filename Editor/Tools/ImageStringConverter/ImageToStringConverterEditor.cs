...
namespace BluLib.Internal
{
	...
	{
		[MenuItem("Tools/BluLib/Image To String Converter", false, 50)]
		private static void CreateWindow()
		{
			ImageToStringConverterEditor window = GetWindow<ImageToStringConverterEditor>(false, "Image To String Converter");
			window.minSize = new(400, 300);
			window.maxSize = new(400, 800);
			window.Show();
		}
		...
		private void OnGUI()
		{
			...
			string[] selected = BluGUI.DropAreaPaths("Drag Texture", 60);
			GUI.enabled = false;
			...
			BluEditor.CopyToClipboard(_representation);
			ShowNotification(new(selected[0] + "\nCopied to Clipboard as string"));
