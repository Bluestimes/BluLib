...
namespace BluLib.Internal
{
	...
	public class SeparatorAttributeDrawer : DecoratorDrawer
	{
		...
		public override void OnGUI(Rect position)
		{
			...
			{
				Vector2 textSize = GUI.skin.label.CalcSize(new(title));
				float separatorWidth = (position.width - textSize.x) / 2 - 5;
				position.y += 19;

				GUI.Box(new(position.xMin, position.yMin, separatorWidth, 1), string.Empty);
				GUI.Label(new(position.xMin + separatorWidth + 5, position.yMin - 10, textSize.x, 20), title);
				GUI.Box(new(position.xMin + separatorWidth + 10 + textSize.x, position.yMin, separatorWidth, 1), "");
			}
		}
	}
}
