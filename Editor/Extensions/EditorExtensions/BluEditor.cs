	...
	public static class BluEditor
	{
		...
		private static void ObjectRename()
		{
			if (EditorApplication.timeSinceStartup >= _renameTimestamp)
			{
				...
				MethodInfo renameMethod = type.GetMethod("RenameGO", BindingFlags.Instance | BindingFlags.NonPublic);
		...
		{
			GameObject instanceRoot = PrefabUtility.FindRootGameObjectWithSameParentPrefab(instance);

			GameObject targetPrefab = PrefabUtility.GetCorrespondingObjectFromSource(instanceRoot);
		...
		public static void SetEditorIcon(this GameObject gameObject, bool textIcon, int iconIndex)
		{
			...
			object[] args = new object[] { gameObject, icons[iconIndex].image };
			MethodInfo setIconMethod = egu.GetMethod("SetIconForObject", flags, null, new[] { typeof(Object), typeof(Texture2D) }, null);
		...
		{
			MonoBehaviour[] allObjects = GetAllBehavioursInScenes();
		...
		{
			Object[] allObjects = GetAllUnityObjects();
		...
		{
			MonoBehaviour[] allObjects = root.GetComponentsInChildren<MonoBehaviour>();
		...
		{
			Type desiredAttribute = typeof(T);
			List<ObjectField> result = new();
		...
		public static MonoBehaviour[] GetAllBehavioursInScenes()
		{
			...
			for (int i = 0; i < SceneManager.sceneCount; i++)
			{
				...
				foreach (GameObject gameObject in root)
				{
					...
					foreach (MonoBehaviour behaviour in behaviours) components.Add(behaviour);
				}
			}
		...
		{
			string assetsPath = GetRelativeScriptAssetsPath(so);
		...
		{
			string assetsPath = GetRelativeScriptAssetsPath(mb);
			return new FileInfo(assetsPath).DirectoryName;
		}
