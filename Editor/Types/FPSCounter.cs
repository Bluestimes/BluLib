...
#pragma warning restore 0649
	...
	private void Awake()
	{
		...
		_goodColor = new(.4f, .6f, .4f);
		_okColor = new(.8f, .8f, .2f, .6f);
		_badColor = new(.8f, .6f, .6f);

		int percent = _targetFrameRate / 100;
		int percent10 = percent * 10;
		int percent40 = percent * 40;
		...
		yPos += _yOffset;
		int yPos2 = yPos + 18;
		_rect1 = new(xPos, yPos, linesWidth, linesHeight);
		_rect2 = new(xPos, yPos2, linesWidth, linesHeight);
